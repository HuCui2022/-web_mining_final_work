...
class GeneralizedRCNNTransform(nn.Module):
    ...
    model.
    再将input,target 送入之前，不惜进行标准化处理等。
    ...
    def resize(self, image, target):
        ...
        if self.training:# 如果是训练的话，随机进行缩放大小。
        ...
        bbox = target["boxes"]
        # 根据图像的缩放比例来缩放bbox
        bbox = resize_boxes(bbox, (h, w), image.shape[-2:])  #h,w是之前的图像的高宽，最后的参数是图像缩放后的高宽。bbox是缩放前的标签。
        target["boxes"] = bbox
    ...
    def max_by_axis(self, the_list):
        ...
        for sublist in the_list[1:]:
            for index, item in enumerate(sublist):# sublist 对应每张图像， 那么index,item对应每张图像的c,h,w.
                maxes[index] = max(maxes[index], item) # 然后用maxs[index] 和item 来进行比较，得到较大的。
        return maxes
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        # images【0】.new_full(size,0)，返回的张量的尺度为size，填充为0 大小和类型以及在哪个gpu上，都是与image[0]相同的。
