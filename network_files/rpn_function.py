...
class AnchorsGenerator(nn.Module):
    ...
    Arguments:
        sizes (Tuple[Tuple[int]]):元组的元组。（（a，），（b，），（c，））
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device="cpu"):
        ...
        hs = (h_ratios[:, None] * scales[None, :]).view(-1)  # 1x15

        # left-top, right-bottom coordinate relative to anchor center(0, 0)
        # 生成的anchors模板都是以（0, 0）为中心的, shape [len(ratios)*len(scales), 4]
        # 在第一维stack，就是将相对应的元素组成list，然后这些list组成tensor。
        base_anchors = torch.stack([-ws, -hs, ws, hs], dim=1) / 2  # 15x4
    ...
    def set_cell_anchors(self, dtype, device):
        ...
        if self.cell_anchors is not None:
            ...
            if cell_anchors[0].device == device:  # 只有一张特征图，那么就只有一个cell_anchor[0]
                return
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            # 每个特征图的大小，与原图对应的stride，以及基础的base_anchors
            ...
            # shitfs 表示特征图的每个点在原图上的对应点的坐标位置，因为是缩放了的，所以在原图上体现的是话，就是跳跃stride的方式。
            ...
            # 然后用torch.meshgrid 函数将两个一维的序列生成两个对应的二维图，两个二维图对应的点进行配对，就得到了网格图。也就是特征图在原图上的映射。
            ...
            shifts_anchor = shifts.view(-1, 1, 4) + base_anchors.view(1, -1, 4)  #以shifts为原点，进行偏移。广播进行，使得每个点，所有的baseanchor都可以。
    ...
    def cached_grid_anchors(self, grid_sizes, strides):
        # type: (List[List[int]], List[List[Tensor]])
        # gride_size ， strides  对应特征图的大小，stride 对应原图的比例。（h,w）分别对应。
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 计算特征层上的一步等于原始图像上的步长，原始图像尺寸唯一，一个batch都一样的。，但是特征图有多张。
        ...
        anchors = [torch.cat(anchors_per_image) for anchors_per_image in anchors]   # 将所有特征图得到的anchor cat到一起。
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None):
        ...
        objectness, pred_bbox_deltas = self.head(features)  #产生预测框和预测类别（二分类，只表示是否是前景或者背景）
